...
router.post('/launch', authenticateToken, (req, res) => {
    ...
    db.get('SELECT status FROM accounts WHERE id = ?', [accountId], (err, row) => {
        if (err || !row) return res.status(404).send({ error: 'Account not found' });
        
        if (row.status === 'active') {
            return res.send({ message: 'Session Active' });
        }
        ...
        db.run(`UPDATE accounts SET status = 'active', session_start = ? WHERE id = ?`,
            [now.toISOString(), accountId],
            (err) => {
                if (err) return res.status(500).send({ error: 'Failed to launch' });
                res.send({ message: 'Session Launched' });
            }
        );
    });
});
...
router.post('/place', authenticateToken, async (req, res) => {
    try {
        ...
        if (account.status !== 'active') {
            return res.status(400).send({ error: 'Account is not active' });
        }
