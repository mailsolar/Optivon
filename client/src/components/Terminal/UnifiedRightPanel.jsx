...
export default function UnifiedRightPanel({
    ...
    // NIFTY = 50, BANKNIFTY = 15, FINNIFTY = 25 (Standard conventions)
    const lotSize = selectedSymbol === 'BANKNIFTY' ? 15 : (selectedSymbol === 'NIFTY' ? 65 : (selectedSymbol === 'FINNIFTY' ? 65 : 1));
    ...
    const handlePlaceOrder = async (overrideSide = null) => {
        ...
        // ONE CLICK AUTO-SL LOGIC
        // If One Click is ON and SL is missing, force a default 5% SL to satisfy backend risk rules
        if (oneClickEnabled && (!finalSl)) {
            const defaultRisk = calculateSLTP(finalSide, currentPrice, 5, 5); // 5% Risk
            finalSl = parseFloat(defaultRisk.stopLoss);
            // Optional: finalTp = parseFloat(defaultRisk.takeProfit);
        }

        // VALIDATION STEP
        // Only run strict validation if NOT in one-click mode or if critical fields are missing
        if (!oneClickEnabled && (orderType === 'market' || orderType === 'limit')) {
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col p-3 space-y-3">
                            ...
                            <div className="space-y-3">
                                ...
                                {/* Detailed Order settings only if NOT One-Click */}
                                {!oneClickEnabled && (
                        ...
                        {/* FINAL ACTION SECTION - Pinned to Bottom */}
                        <div className="shrink-0 p-3 pt-0 bg-surface border-t border-border/50">
                            {oneClickEnabled ? (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in zoom-in duration-300 mt-3">
                                    <button
