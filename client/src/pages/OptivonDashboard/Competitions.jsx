        ...
        <div className="flex flex-col gap-10 max-w-6xl mx-auto font-sans">
            ...
            <div className="relative rounded-premium p-12 overflow-hidden bg-surface border border-accent/20 shadow-2xl group">
                ...
                <div className="relative z-10 flex flex-col xl:flex-row justify-between gap-12">
                    ...
                    {/* Stats / Graphic Side */}
                    <div className="w-full xl:w-96 bg-background/50 backdrop-blur-md rounded-premium border border-black/15 p-8 flex flex-col gap-6 shadow-inner">
                        <div className="flex justify-between items-center pb-6 border-b border-black/15">
                            ...
                            {[1, 2, 3].map((rank, i) => (
                                <div key={i} className="flex items-center justify-between group/rank">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-8 h-8 rounded-instrument flex items-center justify-center text-xs font-black border ${rank === 1 ? 'bg-accent/10 border-accent/20 text-accent' : 'bg-white/5 border-black/15 text-muted'}`}>
                                            {rank}
                                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {competitions.map((comp) => (
                    <div key={comp.id} className="group bg-surface rounded-premium p-8 border border-black/15 hover:border-accent/30 transition-all relative overflow-hidden shadow-xl">
                        ...
                        <div className="flex justify-between items-start mb-10 relative z-10">
                            <div>
                                <span className={`inline-block px-3 py-1 rounded-instrument text-[9px] uppercase font-black tracking-widest mb-4 border ${comp.status === 'Active' ? 'bg-accent/10 text-accent border-accent/20' : 'bg-white/5 text-muted border-black/15'}`}>
                                    {comp.status}
                                </span>
                                <h3 className="text-2xl font-bold text-primary uppercase tracking-tight">{comp.title}</h3>
                            </div>
                            <div className="w-12 h-12 bg-background border border-black/15 rounded-premium flex items-center justify-center group-hover:bg-accent group-hover:text-background transition-all shadow-soft">
                        ...
                        <button className="w-full py-4 bg-background border border-black/15 text-muted font-black text-[10px] uppercase tracking-[0.2em] rounded-instrument hover:bg-accent/10 hover:text-accent hover:border-accent/30 transition-all">
